<template>
  <a-menu :mode="mode" :selected-keys="activeMenu" :open-keys="defaultOpenKeys" :auto-open-selected="autoOpenSelected"
    :accordion="appStore.menuAccordion" :breakpoint="appStore.layout === 'mix' ? 'xl' : undefined"
    :trigger-props="{ animationName: 'slide-dynamic-origin' }" :collapsed="!isDesktop ? true : appStore.menuCollapse"
    :style="menuStyle" @menu-item-click="onMenuItemClick" @sub-menu-click="onSubMenuClick" @collapse="onCollapse">
    <MenuItem v-for="(item, index) in sidebarRoutes" :key="item.name" :item="item">
    </MenuItem>
  </a-menu>
</template>

<script setup lang="ts">
import { ref, computed, defineAsyncComponent, watch, type CSSProperties, nextTick } from 'vue'
